...
import com.ampznetwork.chatmod.ChatModCore;
...
import com.ampznetwork.chatmod.model.abstr.ChannelConfigProvider;
import com.ampznetwork.chatmod.model.abstr.ChatDispatcher;
import com.ampznetwork.chatmod.model.abstr.ChatModConfig;
import com.ampznetwork.chatmod.model.abstr.PacketCaster;
import com.ampznetwork.chatmod.model.abstr.PlayerAdapter;
import com.ampznetwork.chatmod.model.exception.CommandException;
import com.ampznetwork.chatmod.model.exception.PermissionException;
...
public class ChatModLiteSpigot extends JavaPlugin implements ChatModConfig, ChatDispatcher, ChannelConfigProvider, PlayerAdapter, PacketCaster, Listener {
    ChatModCore       core;
    ...
    public ChatModLiteSpigot() {
        this.core = new ChatModCore(this, this, this, PermissionAdapter.spigot(), this, this, new SpigotRecipientSerializer(this));
    }
                        ...
                        case "channel", "ch":
                            switch (args[0]) {
                                ...
                                default:
                                    ...
                                    core.activeChannels(player).forEach(chl -> chl.getPlayerIDs().remove(player.getUuid()));
                                    channelOpt.get().getPlayerIDs().add(player.getUuid());
                            }
                            break;
                        case "shout", "sh":
                ...
                case 2:
                    switch (label) {
                        case "channel", "ch":
                            ...
                            switch (args[0]) {
                                case "info":
                                    core.execAndRespond(player, fallbackResponder(sender), () -> core.info(player, channelName));
                                    break;
                                case "join":
                                    core.execAndRespond(player, fallbackResponder(sender), () -> core.join(player, channelName));
                                    break;
                                case "spy":
                                    core.execAndRespond(player, fallbackResponder(sender), () -> core.spy(player, channelName));
                                    break;
                            }
                            break;
                        case "shout", "sh":
                            channelName = args[0];
                            var msg = args[1];
                            core.execAndRespond(player, fallbackResponder(sender), () -> core.shout(player, channelName, msg));
                    }
                    break;
                default:
                    switch (label) {
        ...
        return switch (args.length) {
            ...
            case 2 -> switch (alias) {
                case "channel" -> switch (args[0]) {
                    case "join", "spy" -> core.availableChannels(player).map(ChatModules.NamedBaseConfig::getName).toList();
                    default -> emptyList();
                };
                case "shout" -> core.availableChannels(player).map(ChatModules.NamedBaseConfig::getName).anyMatch(args[0]::equals)
                                ? List.of("<message>")
                                : emptyList();
                default -> emptyList();
            };
    ...
    public void onEnable() {
        ...
        rabbit = Rabbit.of(cfg.getString("modules.rabbitmq.rabbitUri", "amqp://guest:guest@localhost:5672")).assertion();
        ...
        formattingScheme = cfg.getString("formatting.scheme", "&7[%server_name%&7] #&6%channel_name%&7 <%player_name%&7> &r%message%");
    ...
    private Player getOrCreatePlayer(org.bukkit.entity.Player player) {
        return players.computeIfAbsent(player.getUniqueId(), k -> new Player(k, player.getName()));
    }
    ...
    public void dispatch(AsyncPlayerChatEvent event) {
        try {
            ...
            if (core.hasAccess(player.getUuid(), channel)) {
    ...
    public void sendToPlayer(ComponentLike component, @NotNull Player player) {
        ...
        var bukkitPlayer = getServer().getPlayer(player.getUuid());
    ...
    private Player requirePlayer(@NotNull CommandSender sender) {
        if (sender instanceof org.bukkit.entity.Player player) return basicPlayer(player);
    ...
    private Component formatMessage(String source, String channelName, OfflinePlayer player, Component content) {
        ...
        var adapter = SoftDepend.type("me.clip.placeholderapi.PlaceholderAPI")
                .map($ -> com.ampznetwork.chatmod.model.abstr.PlaceholderAdapter.Hook)
                .orElse(com.ampznetwork.chatmod.model.abstr.PlaceholderAdapter.Native);
        var prefix = adapter.applyPlaceholders(source, channelName, basic.getName(), basic, fixes[0]);
        var suffix = fixes.length > 1 ? adapter.applyPlaceholders(serverName, channelName, basic.getName(), basic, fixes[1]) : "";
    ...
    private Player basicPlayer(OfflinePlayer player) {
