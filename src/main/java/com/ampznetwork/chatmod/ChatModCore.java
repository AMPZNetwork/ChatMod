package com.ampznetwork.chatmod;
...
import com.ampznetwork.chatmod.lang.Words;
import com.ampznetwork.chatmod.model.JacksonPacketConverter;
...
public class ChatModCore implements ChannelConfigProvider {
    ...
    public void localcast(Channel channel, Component component) {
        ...
        channel.allPlayerIDs()
                ...
                .filter(player -> hasAccess(player.getUuid(), channel))
    ...
    public void requireAnyPermission(@Nullable Player player, @NotNull String perm) {
        if (player == null)
            ...
            return;
        var result = permissionAdapter.checkPermissionOrOp(player.getUuid(), perm, true);
    ...
    public Stream<Channel> availableChannels(Player player) {
        return channelProvider.getChannels().stream().filter(channel -> hasAccess(player.getUuid(), channel));
    }
    ...
    public Component shout(@NotNull Player player, String channelName, String msg) {
        ...
        var basicPlayer = new Player(playerId, playerName);
    ...
    public ChatMessage createJoinMessage(Player player) {
        var playerName = player.getName();
        return new ChatMessage(new Player(player.getUuid(), playerName),
                playerName,
